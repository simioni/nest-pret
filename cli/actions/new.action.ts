...
import { generateInput, generateSelect } from '../questions/questions';
import { defaultGitIgnore } from '../configuration/defaults';
import {
  AbstractCollection,
  Collection,
  CollectionFactory,
...
const askForMissingInformation = async (inputs: Input[], options: Input[]) => {
  ...
  if (!nameInput!.value) {
    ...
    const questions = [generateInput('name', message)('pret-app')];
 ...
 * @return Resolves when succeeds, or rejects with any error from `fn.rename`.
 */
const renameDotEnvDotExample = (dir: string) => {
  const filePath = join(process.cwd(), dir, '.env.example');
  const newPath = join(process.cwd(), dir, '.env');
 ...
 */
const createEmptyDistDirectory = (dir: string) => {
  const distPath = join(process.cwd(), dir, '/dist');
